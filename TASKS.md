...
- [ ] **1.1 Scaffold.** Vite + TS strict, ESLint flat config + Prettier, Vitest, folder skeleton per `TECH_SPEC.md` §3, `npm run verify` script, git init, first commit. *Done when: `npm run verify` passes on an empty project and the repo layout matches the spec.*
