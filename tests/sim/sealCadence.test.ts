...
/** A survivable World stepper — arcadeCadenceS (75s) is long enough that a Brush
 *  standing perfectly still dies to ordinary Blot attrition well before the first
 *  automatic Seal encounter could ever fire, which would tell this file nothing about
 *  cadence/cycling. The `mixed` harness bot (the same one `npm run sim` itself drives)
 *  survives that stretch, so it's used here purely as a "keep the Passage alive" input
 *  source — its own targeting strategy is irrelevant to what this file tests. */
